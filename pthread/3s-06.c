...
void *count3s_thread(void *arg) {
	...
	t02 = clock();
	times[id]= (((float)t02 - (float)t01) / 1000000.0F ) * 1000;
}
...
int main(int argc, char *argv[]) {
	...
	while (i < max_threads) {
		...
		if (err != 0) 
			printf("[3s-06] Can't create a thread: [%d]\n", i);
		else
			printf("[3s-06] Thread created!\n");
		i++;
	}
	...
	printf("[3s-06] Count by threads %d\n", count);
	printf("[3s-06] Double check %d\n", double_count);
	pthread_mutex_destroy(&mutex);
	t2 = clock();
	//printf("[[3s-06] Elapsed time %ld ms\n", ((double)t2 - t1) / CLOCKS_PER_SEC * 1000);
	printf("[[3s-06] Elapsed time %f\n", (((float)t2 - (float)t1) / 1000000.0F ) * 1000);
	...
	printf("[[3s-06] Threads total time %f \n", totalt);
	printf("[[3s-06] Threads max time %f \n", maxt);
